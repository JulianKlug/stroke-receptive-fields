function perf_clinical_ctnorm(V, lesion, applyToOther, vox, bb, DeleteIntermediateImages, UseTemplateMask, UseStrippedTemplate, AutoSetOrigin)
% This script attempts to normalize a CT scan
% V = filename[s] of CT scan[s] to normalize. This file is used to
...
% applyToOther = filename[s] of other images the deformation should be
% applied to (ex: perfusion CT maps)
...
% UseTemplateMask = if 1, brain mask from folder is used
...
if nargin < 1 % no other image to apply to 
    applyToOther = spm_select(inf,'image','Optional: select other image to apply this deformation to. (same order as CTs)');
elseif nargin < 3 
    applyToOther = '';
end
...
if (size(applyToOther) > 1) and (size(applyToOther) ~= size(V))
   fprintf('You must specify the same number of other images to apply to as CT scans\n');
   return;
end;
...
if exist('AutoSetOrigin', 'var') && (AutoSetOrigin)
	for i=1:size(V,1)
        ...
        if ~isempty(applyToOther)
 			r = strvcat(r, deblank(applyToOther(i,:))  );
        end
...
for i=1:size(V,1)
    ...
    elseif ~isempty(applyToOther)
       [pthL,namL,extL] = spm_fileparts(deblank(applyToOther(i,:)));
       otherImageName = fullfile(pthL,[namL extL]);
       if (clinical_filedir_exists(otherImageName ) == 0)  %report if files do not exist
        fprintf(' No other image found named:  %s\n', otherImageName );
        return
       end;
       c_other = h2cSub(otherImageName);
       matlabbatch{1}.spm.spatial.normalise.estwrite.subj.wtsrc = '';
       matlabbatch{1}.spm.spatial.normalise.estwrite.subj.resample = {[c_other ,',1']};
       fprintf('Applying the deformation estimated from %s to %s using template %s.\n',ref, otherImageName, cttemplate);

    else % if no lesion and no other images to apply deformation to are specified
        matlabbatch{1}.spm.spatial.normalise.estwrite.subj.wtsrc = '';
